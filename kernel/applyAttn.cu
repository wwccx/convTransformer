__global__ void applyAttn(float* output,
    ...
    if (pixelIdx < H * W && dimIdx < embedDim && batchIdx < B * Heads) {
        ...
        int resoMap = H * W;
        int resoV = resoMap + (win - 1) * (H + W + win - 1);  // (H + win - 1) * (W + win - 1)
        // int resoK = (H + win - 1) * (W + win - 1);
        int u = pixelIdx / W;
        int v = pixelIdx % W;

        int mapIdx = batchIdx * win * win * resoMap + pixelIdx;
        int vIdx = (batchIdx * embedDim + dimIdx) * resoV + u * (W + win - 1) + v;

        float sum = 0;
        for (int i = 0; i < win*win; i++) {
            sum += attnMap[mapIdx + i * resoMap] * V[vIdx + (i / win) * (W + win - 1) + i % win];
...
void launch_applyAttn(float* output,
    ...
    dim3 block(32, 1, 32);
    applyAttn<<<grid, block>>>(output, attnMap, v, B, Heads, win, C, H, W);
}