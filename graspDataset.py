...
class GraspDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_dir, pattern='train', batch_size=32, index_split=0.9):
        ...
        self.img_std = np.load(os.path.join(dataset_dir, 'im_std.npg'))
        self.img_mean = np.load(os.path.join(dataset_dir, 'im_mean.npg'))
        self.pose_std = np.load(os.path.join(dataset_dir, 'pose_std.npg'))
        self.pose_mean = np.load(os.path.join(dataset_dir, 'pose_mean.npg'))
    ...
    def __getitem__(self, item):
        ...
        if self.dataset_partten == 'train':
            ...
            choice_index = np.random.choice(img.shape[0] - index_range, 8) + index_range
        grasp = grasp[choice_index]
        img = img[choice_index]
        metric = metric[choice_index]
        angle = grasp[:, 3]
        depth = grasp[:, 2]
        depth = (depth - self.pose_mean) / self.pose_std
        img = (img.transpose((0, 3, 1, 2)) - self.img_mean)/self.img_std
        ...
        mask[int(angle // (np.pi / angular_bins)) * 2] = 1
        mask[int(angle // (np.pi / angular_bins)) * 2 + 1] = 1

        return img, metric, mask
...
class GraspLossFunction(torch.nn.Module):
    ...
    def forward(self, input, target, mask):
        input = input.squeeze()
        valid_input = input[torch.where(mask > 0)].view(-1, 2)
...
if __name__ == '__main__':
    ...
    import time
    train_data = D.DataLoader(dataset, batch_size=128//32, num_workers=12)
    print(train_data.__len__())
    num = 0
    t0 = time.time()
    for img, depth, label, metric in train_data:
        print(img.shape)
        # if num % 20 == 0:
